...
def load_model(model_name, dataset_name, augment=False, smooth=0, models=models):
    ...
    elif model_name == "dualtransformer":
        from prediction.model.DualTransformer.interface import DualTransformerInterface
        api_class = DualTransformerInterface
...
def attack(model_name, dataset_name, overwrite=0, mode="single_frame", augment=False, smooth=0, blackbox=False):
    ...
    # datasets: {'scalar_v': 10.539, 'linear_a': 4.957, 'rotate_a': 0.956, 'linear_aa': 8.418, 'rotate_aa': 1.577}
...
def main():
    ...
    # python test/test.py --dataset ngsim --model grip --mode single_frame --augment --smooth 0 --blackbox
    parser.add_argument("--dataset", type=str, default="apolloscape", help="Name of dataset [apolloscape, ngsim, nuscenes, dfl]")
    parser.add_argument("--model", type=str, default="grip", help="Name of model [grip, fqa, trajectron, trajectron_map, dualtransformer]")
