...
#         return res if res else [0]
    ...
    def modify_file(self, features, dir='/tmp', verbose=False):
                ...
                if type(feat_val) == float and abs(features_current[feat] - feat_val) < 1e-6:
                    ...
                    if valid == True:
                        ...
                        if verbose:
                            ...
                            if verbose:
                                print("Attempting to change feature {0} from {1} to {2}".format(feat, features_current[feat], feat_val))
                            if verbose:
                                print("Feature '%s' failed: %s" % (feat, valid))
                        report[feat] = valid
            else:
        ...
        if 'version' in to_change:
            ...
            if match != None:
                if verbose:
                    print("Writing %s to mm[%d]" % (str(to_change['version']), match.start(1)))
        ...
        for feat in _incrementable_feats:
            if feat in to_change:
                ...
                if verbose:
                    print("Inserting '%s'*%d" % (_incrementable_feats[feat], increase))
        ...
        for (feat_prefix, feat_str) in _metadata_feats.iteritems():
            ...
            if verbose:
                print("Inserting '%s'" % feat_str.format(addendum))
        ...
        if 'moddate_ts' in to_change or 'moddate_tz' in to_change:
            ...
            if datelines: 
                ...
                if len(new_moddate) == len(old_moddate):
                    if verbose:
                        print('Changing "%s" to "%s"' % (old_moddate, new_moddate))
            ...
            if not modified:
                ...
                if verbose:
                    print('Inserting "%s"' % moddate.format(ts=ts_str, tz=tz_str))
        ...
        if 'createdate_ts' in to_change or 'createdate_tz' in to_change:
            ...
            if datelines: 
                ...
                if len(new_createdate) == len(old_createdate):
                    if verbose:
                        print('Changing "%s" to "%s"' % (old_createdate, new_createdate))
                    mm.seek(mod_location)
                    mm.write(new_createdate)
                    modified = True
            if not modified:
                if verbose:
                    print('Inserting "%s"' % createdate.format(ts=ts_str, tz=tz_str))
        ...
        if 'size' in to_change:
            ...
            if insert_count > 0:
                if verbose:
                    print('Modifying size by inserting {} bytes'.format(insert_count))
                self._insert_into_mmap(mm, ' ' * insert_count)
                report['size'] = to_change['size']
                del to_change['size']
            else:
                if verbose:
                    print('Not modifying size [{} extra bytes already]'.format(-insert_count))
